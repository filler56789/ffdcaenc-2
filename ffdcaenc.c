...
int32_t dcaenc_channel_config_to_count(int32_t channel_config)
{
...
dcaenc_context dcaenc_create(int32_t sample_rate, int32_t channel_config,
                             int32_t approx_bitrate, int32_t flags)
{
	int32_t i, frame_bits, bit_step, fir, useful_bitrate, min_frame_bits;
	...
	if ( useful_bitrate * 512 % sample_rate != 0 ) {
		frame_bits = div_round_up(useful_bitrate * 512, sample_rate);
	}
	else {
		frame_bits = useful_bitrate * 512 / sample_rate;
	}
	//fprintf(stderr, "useful_bitrate: %d\n", useful_bitrate);
	//fprintf(stderr, "frame_bits: %d\n", frame_bits);
...
static inline const int32_t *pcm_sample(dcaenc_context c,
                                        ...
                                        int32_t sample, int32_t channel)
{
...
static void adjust_jnd(int32_t samplerate_index,
...
typedef void (*walk_band_t)(dcaenc_context c, int32_t band1, int32_t band2, int32_t f,
                            int32_t spectrum1, int32_t spectrum2, int32_t channel,
                            int32_t * arg);

static void walk_band_low(dcaenc_context c, int32_t band, int32_t channel,
...
static void walk_band_high(dcaenc_context c, int32_t band, int32_t channel,
                           walk_band_t walk, int32_t * arg)
{
	int32_t f;
	if (band == 31) {
...
static void walk_whole_spectrum(dcaenc_context c, int32_t channel,
                                walk_band_t walk, int32_t * arg)
{
	int32_t band;
	for (band = 0; band < 32; band++)
...
static void update_band_masking(dcaenc_context c, int32_t band1, int32_t band2,
							int32_t f, int32_t spectrum1, int32_t spectrum2,
							int32_t channel, int32_t * arg)
{
...
static const int32_t snr_fudge = 128;
static const int32_t USED_1ABITS = 1;
static const int32_t USED_NABITS = 2;
static const int32_t USED_26ABITS = 4;

static int32_t init_quantization_noise(dcaenc_context c, int32_t noise)
{
	int32_t ch, band;
	int32_t ret = 0;
	...
	for (ch = 0; ch < c->fullband_channels; ch++) {
		for (band = 0; band < 32; band++) {
			int32_t snr_cb = c->peak_cb[band][ch]
...
static void dcaenc_assign_bits(dcaenc_context c)
{
	...
	int32_t low, high, down;
	int32_t used_abits = 0;
...
static void bitstream_put(dcaenc_context c, uint32_t bits, int32_t nbits)
{
	int32_t max_bits = (c->flags & DCAENC_FLAG_28BIT) ? 28 : 32;
...
static int32_t dcaenc_quantize(dcaenc_context c, int32_t sample, int32_t band, int32_t ch)
{
...
static int32_t dcaenc_calc_one_scale(int32_t peak_cb, int32_t abits, softfloat *quant)
{
	int32_t peak;
	int32_t our_nscale, try_remove;
	softfloat our_quant;
...
static void put_frame_header(dcaenc_context c, int32_t normal)
{
	/* SYNC */
	...
	//fprintf(stderr, "fsize in header: %d\r", c->frame_bits / 8 - 1);
	...
	/* Source PCM resolution: 24 bits (even though the library accepts only 32-bit samples), not DTS ES */
	/* This should really be passed in the call to dcaenc, based on the input file, but I didn't want to change the API */
	//bitstream_put(c, 0, 3);	//98 16 bit source
	bitstream_put(c, 6, 3);	//98
...
static void put_primary_audio_header(dcaenc_context c)
{
	...
	/* Joint32_t intensity coding index: 0 */
	for (ch = 0; ch < c->fullband_channels; ch++)
...
static void put_subframe_samples(dcaenc_context c, int32_t ss, int32_t band, int32_t ch)
{
	if (c->abits[band][ch] <= 7) {
		...
		int32_t i;
		for (i = 0; i < 8; i++) {
			int32_t bits = bit_consumption[c->abits[band][ch]] / 16;
...
static void put_subframe(dcaenc_context c)
{
	...
	/* Joint32_t subband scale factor codebook select: not transmitted */
	/* Scale factors for joint32_t subband coding: not transmitted */
...
static int32_t dcaenc_convert_frame(dcaenc_context c, const int32_t *input, uint8_t *output, int32_t normal)
{
	int32_t i;

	int32_t l = dcaenc_output_size(c);
	for (i = 0; i < l; i++)
		output[i] = 0;
...
{
	int32_t retval = output ? dcaenc_convert_frame(c, zero, output, 0) : 0;
	free(c);
	return retval;
}
